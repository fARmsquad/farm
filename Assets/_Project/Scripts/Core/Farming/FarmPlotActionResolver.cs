...
namespace FarmSimVR.Core.Farming
{
    ...
    public static class FarmPlotActionResolver
    {
            ...
            int lettuceSeeds)
        {
            ...
            string title = $"{PrettyPlotName(soil.PlotId)} [{soil.Status}]";
            string detail = BuildDetail(soil, crop);
            return new FarmPlotActionPrompt(title, detail, actions);
        }
        ...
        private static string BuildDetail(SoilState soil, CropPlotState crop)
        {
            string cropLabel = soil.CurrentCropId == null
                ? soil.Type + " soil"
                : soil.CurrentCropId.Replace("seed_", string.Empty);

            return $"{cropLabel}   Moisture {soil.Moisture:P0}   Nutrients {soil.Nutrients:P0}   Growth {crop.GrowthPercent:P0}";
        }
    }
}
